...
-- This SQL query selects the Location and the maximum Total_deaths (casted as INT) as TotalDeathCount from the CovidDeaths table.
-- It filters the data to include records where the continent column is not NULL.
-- The results are grouped by Location.
-- The WHERE clause has been commented out, so the query calculates the maximum total death count for all locations.
...
-- This SQL query selects the continent and the maximum Total_deaths (casted as INT) as TotalDeathCount from the CovidDeaths table.
-- It filters the data to include records where the continent column is not NULL.
...
-- It sums up the new_cases and casts new_deaths as INT to get the total cases and total deaths, respectively.
...
-- The WHERE clause has been commented out, so the query calculates these metrics for all continents.
...
SELECT 
	...
	OVER (Partition by dea.Location ORDER BY by dea.location, dea.Date) AS RollingPeopleVaccinated
